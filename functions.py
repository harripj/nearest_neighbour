...
def nearest_neighbour_distribution_2D_impenetrable(r, sigma, rho):
    '''
    Produce expected NND probability distribution in 2D given a random (Poisson) arrangement
    of particles (hard disks in 2D).

    NB. distribution is valid for r > sigma.

    Equation obtained from S Torquato et al 1990 J. Phys. A: Math. Gen. 23 L103.
...
def parse_multiple_nnd_from_JSON(json_list, knn=1, plot=True, 
                                 scale_data=False):
    '''
